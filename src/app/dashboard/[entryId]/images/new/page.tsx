import { CreateImage } from "@/components/memorials/create-image";
import { ImageResult } from "@/components/memorials/results";
...
import { getEntryById, getUserSavedQuotes } from "@/lib/db/queries";
...
export default async function Create({
  ...
  if (!session?.user) redirect("/auth/login?redirectTo=/dashboard");
  ...
  const createEpitaphsAction = async (formData: any, userId: string) => {
    "use server";
    return createEpitaphs(formData, userId, entryId);
  };
